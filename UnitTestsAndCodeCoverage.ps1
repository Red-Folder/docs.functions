...
.\packages\OpenCover.4.6.519\tools\OpenCover.Console.exe -register:user -output:TestAndCoverage\coverage.xml -target:"packages\xunit.runner.console.2.3.1\tools\net452\xunit.console.x86.exe" -targetargs:"tests\DocFunctions.Lib.Unit\bin\$BuildConfiguration\DocFunctions.Lib.Unit.dll -noshadow -xml TestAndCoverage\UnitTestResults.xml" -filter:"+[DocFunctions.Lib*]* -[DocFunctions.Lib.Unit]*" -hideskipped:Attribute
